$(function () {
  ...
  $(canvas).css(
    " border-left: 30px; border-radius: 3%; border: thick double #32a1ce;"
  );
  ...
  socket.on("connected", function (SERVER_GAME_PARAMS) {
    GAME_PARAMS = SERVER_GAME_PARAMS;
    $(canvas).attr("width", GAME_PARAMS.WIDTH);
    $(canvas).attr("height", GAME_PARAMS.HEIGHT);
    // $(canvas).attr("border", "thick double #32a1ce");
    document.body.appendChild(canvas);
    start.initialize();
  });
  socket.on("new user entered", function () {
    new Audio("/wav/notification001.wav").play();
    //new Audio("/wav/newchallenger.mp3").play();
  });
  socket.on("total user count updated", function (count) {
    window.document.title = GAME_PARAMS.TITLE + " (" + count + ")";
  });
  ...
  function drawBackground(globalAlpha, color) {
    ...
    ctx.fillStyle = color ? color : GAME_PARAMS.BACKGROUND_COLOR;
    ctx.fillRect(0, 0, GAME_PARAMS.WIDTH, GAME_PARAMS.HEIGHT);
    ctx.restore();
  }

  function drawBorder() {
    ctx.fillStyle = "#000000";
    ctx.fillRect(0, 0, GAME_PARAMS.WIDTH, GAME_PARAMS.BORDER_WIDTH);
    ctx.fillRect(
      0,
      GAME_PARAMS.HEIGHT - GAME_PARAMS.BORDER_WIDTH,
      GAME_PARAMS.WIDTH,
      GAME_PARAMS.BORDER_WIDTH
    );
  }

  function drawNet() {
    var num = 10;
    var height = GAME_PARAMS.HEIGHT / ((num + 1) * 2);
    var y = height / 2;
    var x = (GAME_PARAMS.WIDTH - GAME_PARAMS.NET.WIDTH) / 2;
    ctx.fillStyle = "#f7f7f7";
    while (y < GAME_PARAMS.HEIGHT) {
      ctx.fillRect(x, y, GAME_PARAMS.NET.WIDTH, height);
      y += height * 2;
    }
  }
  ...
  var start = new (function () {
    ...
    start.initialize = function () {
      start.opening.initialize(canvas, ctx, GAME_PARAMS);
      start.button1.initialize(canvas, ctx, GAME_PARAMS, {
        text: {
          x: undefined,
          y: 400,
          size: 30,
          ...
          colorData: {
            default: { fill: "#c91c3c", stroke: undefined },
            mouseover: { fill: "#b7d0e8", stroke: undefined },
          },
        },
        rect: {
          x: undefined,
          ...
          color: { fill: undefined, stroke: undefined },
          colorData: {
            default: { fill: "#8ac91c", stroke: "#5b80a6" },
            mouseover: { fill: "#0686e0", stroke: "#a5aeb8" },
          },
        },
        animation: {
          maxCount: 100,
    ...
    start.destroy = function () {
      ...
      canvas.removeEventListener(
        "touchstart",
        ButtonObject.prototype.events.touchstart
      );
      canvas.removeEventListener(
        "touchmove",
        ButtonObject.prototype.events.touchmove
      );
      canvas.removeEventListener(
        "touchend",
        ButtonObject.prototype.events.touchend
      );
    };
  ...
  var waiting = new (function () {
    ...
    waiting.initialize = function () {
      waiting.text1.initialize(canvas, ctx, GAME_PARAMS, {
        text: {
          x: undefined,
          y: undefined,
          size: 30,
          ...
          color: { fill: undefined, stroke: undefined },
          colorData: {
            default: { fill: "#fffafa", stroke: undefined },
          },
        },
        animation: {
          maxCount: 100,
  ...
  var ready = new (function () {
    ...
    ready.initialize = function (position) {
      ...
      switch (position) {
        case "left":
          message = "MOI";
          xPos = (GAME_PARAMS.WIDTH * 1) / 4;
          break;
        case "right":
          message = "MOI";
          xPos = (GAME_PARAMS.WIDTH * 3) / 4;
          break;
      }

      ready.text1.initialize(canvas, ctx, GAME_PARAMS, {
        text: {
          x: xPos,
          y: undefined,
          size: 25,
      ...
      ready.text2.initialize(canvas, ctx, GAME_PARAMS, {
        text: {
          x: undefined,
          y: GAME_PARAMS.HEIGHT - 80,
          size: 20,
          font: "Arial",
          textBaseline: "middle",
          textAlign: "center",
          lineWidth: 1,
          message: "CLICK  [PRET]    POUR COMMENCER LA PARTIE ! ",
          globalAlpha: undefined,
          color: { fill: undefined, stroke: undefined },
          colorData: {
            default: { fill: "#cf1515", stroke: "#cf1515" },
          },
        },
      });
      ready.button1.initialize(canvas, ctx, GAME_PARAMS, {
        rect: {
          x: xPos,
          y: GAME_PARAMS.HEIGHT / 2 + 40,
          width: 150,
          height: 40,
          lineWidth: 2,
          color: { fill: undefined, stroke: undefined },
          colorData: {
            default: { fill: "#37e817", stroke: undefined },
            mouseover: { fill: "#1f4d16", stroke: undefined },
          },
        },
        text: {
          x: xPos,
          ...
          color: { fill: undefined, stroke: undefined },
          colorData: {
            default: { fill: "#fdfffc", stroke: undefined },
            mouseover: { fill: "#fffcfe", stroke: undefined },
          },
        },
        animation: {
          maxCount: 100,
  ...
  var playing = new (function () {
    ...
    playing.events.canvas = function (e) {
      var x =
        e.changedTouches[0].clientX - e.changedTouches[0].target.offsetLeft;
      var y =
        e.changedTouches[0].clientY - e.changedTouches[0].target.offsetTop;
      e.preventDefault();
    ...
    playing.events.body = function (e) {
      ...
      if (
        e.changedTouches[0].clientY > canvasTop &&
        e.changedTouches[0].clientY < canvasBottom
      ) {
        e.preventDefault();
  ...
  var backToTitle = new (function () {
    ...
    backToTitle.text2.update = function () {
      ...
      if (animation.count > 100)
        text.globalAlpha = (animation.count - 100) / 150;
    };

    backToTitle.initialize = function (message) {
      backToTitle.count = 0;
      backToTitle.text1.initialize(canvas, ctx, GAME_PARAMS, {
        text: {
          x: undefined,
          y: GAME_PARAMS.HEIGHT / 2 - 20,
          size: 32,
          font: "Arial",
          textBaseline: "middle",
          textAlign: "center",
          ...
          globalAlpha: undefined,
          color: { fill: undefined, stroke: undefined },
          colorData: { default: { fill: "#f7f7f7", stroke: undefined } },
        },
        animation: {
          maxCount: 200,
          count: 0,
      ...
      backToTitle.text2.initialize(canvas, ctx, GAME_PARAMS, {
        text: {
          x: undefined,
          y: GAME_PARAMS.HEIGHT / 2 + 20,
          size: 25,
    ...
    backToTitle.update = function () {
      if (backToTitle.count == 0) {
        drawBackground(0.8);
        backToTitle.imgData = ctx.getImageData(
          0,
          0,
          GAME_PARAMS.WIDTH,
          GAME_PARAMS.HEIGHT
...
TextObject.prototype.initialize = function (canvas, ctx, GAME_PARAMS, data) {
  this.canvas = canvas;
  this.ctx = ctx;
  ...
  text.x = text.x ? text.x : GAME_PARAMS.WIDTH / 2;
  text.y = text.y ? text.y : GAME_PARAMS.HEIGHT / 2;
  text.color = text.colorData.default;
};
...
ButtonObject.prototype.initialize = function (canvas, ctx, GAME_PARAMS, data) {
  TextObject.prototype.initialize.call(this, canvas, ctx, GAME_PARAMS, data);
  var rect = this.data.rect;
  var text = this.data.text;
  rect.x = rect.x ? rect.x : text.x ? text.x : GAME_PARAMS.WIDTH / 2;
  rect.y = rect.y ? rect.y : text.y ? text.y : GAME_PARAMS.HEIGHT / 2;
...
function Opening() {}
Opening.prototype.initialize = function (canvas, ctx, GAME_PARAMS) {
  this.ctx = ctx;
  var titleSetting = [
    {
      x: GAME_PARAMS.WIDTH / 2.8,
      y: 100,
      size: 60,
      space: 100,
      lineWidth: 5,
      color: { fill: "#2953cf", stroke: "#f2bbbb" },
      text: "PONG",
    },
    {
      x: GAME_PARAMS.WIDTH / 2 + 80,
      y: 220,
      size: 30,
      space: 60,
      lineWidth: 5,
      color: { fill: "#a3f08d", stroke: "#f7f7f7" },
      text: "by PICATSU",
    },
    {
      x: GAME_PARAMS.WIDTH / 2 + 217,
      y: 165,
      size: 12,
      space: 12,
      lineWidth: 5,
      color: { fill: "#f7f7f7" },
      text: "Ver. " + GAME_PARAMS.VER,
    },
  ];
  this.title = [];
  ...
  this.animationLayer[0].push(
    Down(this.animationLayer[0], this.title[0], 50, 500)
  );

  this.animationLayer[1] = [Hide(this.animationLayer[1], this.title[1], 0)];
  this.animationLayer[1].push(
    Down(this.animationLayer[1], this.title[1], 150, 500)
  );

  this.animationLayer[2] = [Hide(this.animationLayer[2], this.title[2], 0)];
  this.animationLayer[2].push(
    FadeIn(this.animationLayer[2], this.title[2], 250, 500)
  );
  ...
  function generateTitleText(title) {
    ...
    for (var i = 0; i < title.text.length; i++) {
      ...
      text.initialize(canvas, ctx, GAME_PARAMS, {
        text: data,
        default: data,
      });
  ...
  function Down(animationLayer, title, startCount, endCount) {
    ...
    return function closure() {
      ...
      if (opening.count >= startCount && opening.count < endCount) {
        for (var i = 0; i < title.length; i++) {
          if (
            opening.count >= actionData[i].startCount &&
            title[i].data.text.y < title[i].data.default.y
          ) {
            actionData[i].speed =
              (title[i].data.default.y - title[i].data.text.y) / 40 + 0.3;
