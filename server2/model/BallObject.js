  ...
  if (this.serve && this.serve.isOn) {
    ...
    if (this.boostCount > 0) {
      ...
      if (this.boostCount > this.boostCountMax / 2) {
        this.status.rect.color.fill = "#f2c624";
        boost = 2 * this.speed;
      } else {
        this.status.rect.color.fill = "#f2c624";
    ...
    for (object in room.objects) {
      if (room.objects[object].role == "player") {
        playerStat = room.objects[object].status.rect;
        var collusionType = ballCollusionCheck(
          ball,
          playerStat,
          this.dynamic.angle
        );
    ...
    case 1:
      ...
      if (bLeftRight == p2bLeftRight) {
        ...
        if (
          ...
          type =
            bUpDown != p2bUpDown
              ? COLLUSION_TYPE.SMASH_TYPE_1
              : COLLUSION_TYPE.SMASH_TYPE_2;
        else
          type =
            p2bLeftRight == TO.LEFT
              ? COLLUSION_TYPE.LEFT
              : COLLUSION_TYPE.RIGHT;
      }
      break;
    case 2:
